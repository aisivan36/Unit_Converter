...
/// The rest API retrieves unit conversion for [Categories] that change.
/// For example, the currency exchange rate, stock prices, the height of the tides change often.
/// We have set up an API that retrieves a list of currencies and their current exchange rate (moc data).
...
class Api {
  ...
  /// We specify the type here for readability. Since we are defining a final field, the type is determined at initialization.
  final HttpClient _httpClient = HttpClient();

  ///The API endpoint we want to hit.
  ///This API does not have a key but often, APIs do require authentication
  final String _url = 'flutter.udacity.com';

  ///
  /// Gets all the units and conversion rates for a given category.
  ///
  /// The 'category' parameter is the name of the [Category] from which to retrieve units. We pass this into the query parameter in the API call.
  ///
  /// Returns a list. Return null on error.
  ///
  Future<List> getUnits(String category) async {
    final uri = Uri.https(_url, '/$category');
    final jsonResponse = await _getJson(uri);
    if (jsonResponse == null || jsonResponse['units'] == null) {
      print('an error retrieveing units');
      return null;
    }
  ...
  ///
  /// Return null if the API server is down, or the response is not JSON.
  Future<Map<String, dynamic>> _getJson(Uri uri) async {
    try {
      ...
      final responseBody = await _httpResponse.transform(utf8.decoder).join();

      ///Finally, the string is prse into a JSOn object.
      return json.decode(responseBody);
    } on Exception catch (error_msg) {
      print('$error_msg');
      return null;
    }
  }
